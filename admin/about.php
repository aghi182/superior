    ...
    try {
        ...
        if ($exists) {
            ...
            $stmt = $pdo->prepare("
                ...
                title = ?, description = ?, images = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = 1
            ");
            ...
            // Insert new record
            $stmt = $pdo->prepare("
                INSERT INTO about_content (id, title, description, images)
                VALUES (1, ?, ?, ?)
            ");
            $stmt->execute([$title, $description, $image_url]);
        }
...
// Get current data
try {
    $stmt = $pdo->prepare("SELECT * FROM about_content WHERE id = 1");
    $stmt->execute();
    $about = $stmt->fetch();
} catch (PDOException $e) {
    $error_message = "Database error: " . $e->getMessage();
...
<body>
  ...
  <div class="main-content" id="mainContent">
    ...
    <div class="container mt-4">
      <div class="row">
        <div class="col-12">
          <h2 class="mb-3"><i class="fas fa-info-circle me-2"></i>Edit About Content</h2>
          <p class="text-muted">Manage company about section content</p>
        </div>
      </div>
          ...
          <div class="col-lg-8">
            <div class="card">
              <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i>About Content</h5>
              </div>
              <div class="card-body">
                <div class="mb-3">
                ...
                <div class="mb-3">
                  ...
                  <textarea class="form-control" id="description" name="description" 
                            rows="6" required><?php echo htmlspecialchars($about['description'] ?? ''); ?></textarea>
                </div>
                <div class="mb-3">
                  <label for="image" class="form-label">Image</label>
                  <input type="file" class="form-control" id="image" name="image" 
                         accept="image/*">
                  <?php if ($about && $about['images']): ?>
                  <div class="mt-2">
                    <small class="text-muted">Current: <?php echo htmlspecialchars($about['images']); ?></small>
                    <input type="hidden" name="image_url" value="<?php echo htmlspecialchars($about['images']); ?>">
                  </div>
                  <?php endif; ?>
                </div>
              </div>
          ...
          <div class="col-lg-4">
            <div class="card">
              ...
              <div class="card-body">
                <h6>Current Image:</h6>
                <?php if ($about && $about['images']): ?>
                <img src="<?php echo htmlspecialchars($about['images']); ?>" 
                     alt="About Preview" class="img-fluid rounded mb-3" 
                ...
                <div class="border p-3 rounded bg-light">
                  ...
                  <p><?php echo htmlspecialchars($about['description'] ?? 'Company description will appear here.'); ?></p>
                </div>
              </div>
            </div>
          </div>
